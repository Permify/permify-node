...
export const PermissionLookupSubjectRequest: MessageFns<PermissionLookupSubjectRequest> = {
  encode(message: PermissionLookupSubjectRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.continuousToken !== "") {
      writer.uint32(66).string(message.continuousToken);
    }
    return writer;
  },
...
export interface HealthServiceImplementation<CallContextExt = {}> {
  /** CheckHealth method for health checking */
  check(request: HealthRequest, context: CallContext & CallContextExt): Promise<DeepPartial<HealthResponse>>;
}

export interface HealthClient<CallOptionsExt = {}> {
  /** CheckHealth method for health checking */
  check(request: DeepPartial<HealthRequest>, options?: CallOptions & CallOptionsExt): Promise<HealthResponse>;
}
